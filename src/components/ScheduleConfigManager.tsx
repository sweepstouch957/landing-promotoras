...
const ScheduleConfigManager: React.FC = () => {
  ...
  const defaultConfig: ScheduleConfig = {
    ...
    endDate: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 3 meses
  ...
  const fetchConfigs = async () => {
    ...
    try {
      ...
      const active = result.data?.find((config: ScheduleConfig) => config.isActive);
  ...
  const saveConfig = async () => {
    ...
    try {
      const url = editingConfig._id 
        ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/api/schedule-config/${editingConfig._id}`
        : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/api/schedule-config`;
      ...
      alert(error instanceof Error ? error.message : 'Error al guardar configuración');
    } finally {
      setSaving(false);
    }
  };
  ...
  const activateConfig = async (configId: string) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/api/schedule-config/activate/${configId}`,
        { method: 'POST' }
      );
  ...
  const generateSlots = async (configId: string) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/api/schedule-config/${configId}/generate-slots`,
        { method: 'POST' }
      );
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="body1" sx={{ mb: 3, color: '#666' }}>
        Gestiona los horarios y días disponibles para las reuniones del programa.
      </Typography>
      ...
      {activeConfig && (
        <Alert severity="info" sx={{ mb: 3 }}>
          ...
          <Typography variant="body2">
            ...
            {activeConfig.allowedWeekDays.length} días permitidos • 
            Creación automática: {activeConfig.autoCreateSlots ? 'Habilitada' : 'Deshabilitada'}
          </Typography>
        </Alert>
      )}
      ...
      {configs.length === 0 ? (
        <Alert severity="warning">
          No hay configuraciones creadas. Crea una nueva configuración para comenzar.
        </Alert>
      ) : (
                ...
                <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, width: '100%' }}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {format(new Date(config.startDate), 'dd/MM/yyyy')} - {format(new Date(config.endDate), 'dd/MM/yyyy')}
                    </Typography>
                    <Chip 
                ...
                <AccordionDetails>
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={6}>
                      ...
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                        {config.allowedWeekDays.map(day => (
                          <Chip 
                            key={day}
                            label={diasSemana.find(d => d.value === day)?.label}
                            size="small"
                            color="primary"
                          />
                        ))}
                    ...
                    <Grid item xs={12} md={6}>
                      ...
                      <Typography variant="body2">
                        Zona horaria: {config.timeZone}<br/>
                        Creación automática: {config.autoCreateSlots ? 'Sí' : 'No'}<br/>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                    {diasSemana.map(dia => (
                      <FormControlLabel
                        ...
                        control={
                          <Checkbox
                            checked={editingConfig.allowedWeekDays.includes(dia.value)}
                            onChange={(e) => handleWeekDayChange(dia.value, e.target.checked)}
                            sx={{
                              color: '#ED1F80',
